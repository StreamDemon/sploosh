...
| PID lifetime is independent of references/monitors | `Handle<T>` is not reference-counted either | Both systems share this non-refcounted model: a BEAM process lives until it exits or crashes regardless of who holds its PID, and `Process.monitor/1` is a `:DOWN` notifier rather than a lifetime control. Sploosh's `handle.stop()` is a cooperative shutdown primitive — closest in intent to OTP's `GenServer.stop/3` graceful-stop pattern (drains, then dies) — and is treated as intentional termination by the supervisor (no restart). Note that BEAM `Process.exit(pid, :normal)` does **not** terminate another process (it is a no-op against a foreign PID); `handle.stop()` is **not** that. `handle.kill()` is closest to `Process.exit(pid, :kill)` but with the Sploosh constraint that the *current* handler always runs to completion (no handler interruption). Use supervisors (as in OTP) for failure-driven restart; use `stop()`/`kill()` for the user-driven clean-shutdown path. |
