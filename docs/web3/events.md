...
ABI emission artifacts (bytecode + ABI JSON + metadata) are not yet
specified — the current spec defines the event *shape* and topic layout,
but no compiler output exists yet.
