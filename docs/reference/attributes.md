...
| `@mailbox(capacity: N)` | Set actor mailbox capacity (default 1024). Mailbox stays locked across `.await` points in handlers (§8.10); full mailbox blocks senders. If the destination dies while a sender is blocked, the sender wakes immediately: plain `send` silently drops the message and returns `()`; only `send_timeout` returns `Err(SendError::Dead)`, and request/reply returns `Err(ActorError::Dead)` (§8.11a). Compile error inside `onchain`. |
