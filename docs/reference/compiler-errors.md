...
| `E1119` | compile error | C | reserved | I/O-bound standard library module used inside an `onchain` module. `std::fs`, `std::net`, `std::io`, `std::db`, `std::web`, `std::env`, `std::log`, and `std::time` have no on-chain semantics — there is no filesystem, no network, no stdin/stdout, no database, no HTTP server, no host-process environment, no host log sink, and no host clock (use `ctx::timestamp()` for chain time) in a transaction frame. | §11.1, §12.3 |
