...
7. **Spec fits in a prompt.** The language core is describable compactly enough to be loaded into an LLM system prompt alongside real task context. `docs/spec-plans/LANGUAGE_SPEC_PROMPT.md` is the authoritative prompt-sized artifact; it tracks toward roughly 4,000 tokens (measured with `cl100k_base`) as a soft target, not a hard budget. Amendments that grow the reference should be balanced against pruning or splitting so the artifact remains prompt-loadable; the exact ceiling is a strategy decision revisited as models and deployment contexts evolve.
...
| `overflow-checks` | bool | `true` | `true` | Insert checked-arithmetic guards (§4.7). **Frozen `true` for `evm` and `svm` targets — overrides any user setting and emits warning `W0xxx`.** |
...
fixed by §4.7 / §8 — there is no unwind path to choose).
