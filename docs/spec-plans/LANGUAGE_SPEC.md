...
7. **Spec fits in a prompt.** The language core is describable compactly enough to be loaded into an LLM system prompt alongside real task context. As of v0.5.8 the prompt-sized artifact is split into two files — `docs/spec-plans/LANGUAGE_SPEC_PROMPT_CORE.md` (the language core: syntax, types, ownership, math, actors, errors, modules, runtime, FFI, attributes, testing, diagnostics, manifest, plus a "Common LLM Mistakes" appendix) and `docs/spec-plans/LANGUAGE_SPEC_PROMPT_WEB3.md` (the §11 on-chain surface). Soft per-file targets (measured with `cl100k_base`): roughly 4,000 tokens for `_CORE.md` and roughly 1,500 tokens for `_WEB3.md`, giving consumers the option to load just the audience they need. These remain soft targets, not hard budgets — amendments that grow either reference should be balanced against pruning so each artifact stays prompt-loadable; the exact ceilings are a strategy decision revisited as models and deployment contexts evolve. The retired combined `LANGUAGE_SPEC_PROMPT.md` redirects readers to the split files.
