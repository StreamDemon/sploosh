# SPLOOSH Language Specification v0.5.14-draft
...
### 2.3 Keywords (45 total: 36 reserved + 9 contextual)
...
Suffixes: `i8 i16 i32 i64 i128 u8 u16 u32 u64 u128 u256 f32 f64`. Suffixes are lexical
and attach with no whitespace: `42u32` is one literal token, while `42 u32` is an integer
literal followed by an identifier and is not a suffixed literal. An integer literal
...
`crate`/`super` at path head, `where` after signatures and type headers) never
overlap with an identifier position, and `recv` has no keyword position at all
in the current spec.
...
`where` is contextual and starts a bound clause only after a function signature
or type/trait header. Inline bounds and `where` clauses are semantically
equivalent; `where` is preferred for multi-bound or associated-type constraints.
...
- **Handler-safe FFI.** `extern "C"` blocks may be marked `async` —
  ...
  The `async` marker applies to the whole string-target extern block.
  `extern onchain mod` does not accept `async`.
...
A struct literal (§16 `struct_literal`) may not appear as the outermost
expression of an `if` or `while` condition — `if x { f: 1 } == y { ... }` is
ambiguous with the block head, so parenthesize: `if (x { f: 1 }) == y { ... }`
(Rust's rule).
...
- A struct literal may not appear as the outermost expression of the `match`
  scrutinee (or a `for` iterable) — parenthesize to use one there (§5.1, §16).
...
- When the piped value's type has a method matching the stage name,
  `expr |> method(args)` desugars to `expr.method(args)`. This holds for **any**
  receiver type — iterator adaptors are simply the most common case
  (`Option` methods like `map` / `unwrap_or` pipe the same way, §6.5):
...
**Stage resolution order:** if the piped value's type has a method with the
stage's name, the method wins and the stage lowers to `expr.method(args)`;
otherwise the stage is a free-function call with the piped value as the first
argument (`f(expr, args...)`, the rule above). The two never overlap silently —
a method on the receiver type shadows any same-named free function for that
stage.
...
follows directly from §5.6: `expr |> f(a)` lowers to `f(expr, a)`, and
`expr |> method(args)` lowers to `expr.method(args)` for any receiver whose
type has that method — iterator adaptors are just the common case. Both forms
...
**EBNF:** `actor_def = [ attrs ] "actor" IDENT [ generic_params ] [ where_clause ] "{" [ fields ] { fn_def } "}" ;`
...
`ActorId` is an opaque, `Copy + Eq + Hash + Debug` identifier assigned
at `spawn`. The `Debug` rendering is an opaque, implementation-defined
form (e.g. `ActorId(42)`) intended for logs and diagnostics — not to be
parsed. `ActorId` does not implement `Display`; format it with `{:?}`.
Two distinct actors never share an `ActorId`, and an
...
is not yet specified; deferred to a future amendment.
...
storage-layout implications are not yet specified; deferred to a future
amendment.
...
`offchain` functions are ordinary off-chain code and follow the §8.9 async
rules — `.await` requires the `async` modifier (`offchain async fn`, per the
§16 `fn_def` production).
...
- `std::log` — no host log sink
- `std::time` — no host clock; use `ctx::timestamp()` (§11.2) for chain time
...
`std::crypto`, `std::json`, `std::chain`, `std::collections`, and all core types.
Together with `std::test` (§13.3) and `std::actor` (§8.12.7), covered by the
prohibitions below, these lists are exhaustive against the §13.2 module table:
every §13.2 module whose target column includes neither EVM nor SVM is
forbidden inside `onchain`; every module available on all targets is allowed.
...
| `std::test`    | Test framework, assertions               | native (test), wasm (test) |
...
5. Optionally be `async`. An `@test async fn` runs on a fresh per-test
   runtime (§13.3.5).
...
3. **Actor death** — the handler panicked (failed `assert*`, bounds
   check, overflow, etc.). The runner observes `Err(ActorError::Dead)`
   on the completion channel (§8.8) and records the panic message —
   read from the dead isolation actor's snapshot via §8.12's
   `DeathCause::RuntimeFailure { panic }` — as the failure cause.
...
`@test async fn ...` is permitted. The runner spawns a fresh runtime
...
- Numeric suffixes are part of the literal token and require no intervening
  whitespace. `42u32` is a suffixed `INT_LIT`; `42 u32` tokenizes as `INT_LIT`
  followed by `IDENT`.
...
| §16 grammar tracks promised syntax, not a smaller subset | v0.5.14 repairs the EBNF so the parser accepts the syntax already taught by prose and examples: use-site generic type args, `where` clauses, assignment expressions, inferred closure params, `extern "C" async`, `vec![]`, `assert_matches`, lexical numeric suffixes, and context-specific event declarations. |
...
| Struct literals banned in block-head positions (Rust-style restriction) | The naive `expr = ... \| path_expr "{" field_inits "}"` production makes `if x { }` ambiguous — struct literal vs. condition-plus-block — in every block-head position. Three options were considered: **(1) ban struct literals as the outermost expression of `if`/`while` conditions, `match` scrutinees, and `for` iterables, requiring parens there** (chosen); (2) a cover grammar with parse-then-disambiguate at name resolution — rejected for implementation burden, worse error messages, and unfamiliarity to LLMs; (3) mandatory constructor syntax instead of bare literals — rejected because the spec's examples use bare struct literals everywhere. Option 1 wins on the project's core currency: it is zero-ambiguity with exactly one rule, and it is the rule Rust-trained LLMs already apply without being told. Encoded as a §16 side condition on `struct_literal` plus one-sentence notes in §5.1 / §5.2. |
| `ActorId` gains `Debug` (not `Display`) | Examples across the tree formatted `ActorId` with `{}` while §8.12.5 granted only `Copy + Eq + Hash` — no formatting trait at all. `Debug` is the honest fix: the ID is opaque and implementation-defined (e.g. `ActorId(42)`), so a `Debug` rendering marked not-to-be-parsed supports the actual use case (logs, diagnostics, supervision dumps) without promising a stable user-facing string. `Display` was rejected because `Display` implies a canonical, presentation-grade rendering — granting it would invite parsing and cross-runtime comparison of a value the spec explicitly keeps opaque and runtime-local (§8.12.5, `E1211`). All doc examples now format `ActorId` with `{:?}`. |
| `_CORE` PROMPT ceiling raised `~4,800` → `~5,600` (v0.5.13); `_WEB3` unchanged at `~1,500` | The v0.5.11 grammar repair and v0.5.12 semantic reconciliation added genuine reference surface to `_CORE` — contextual-keyword positions, the pipe-stage `?` rule, struct-literal restriction, `i128`/`u128`, and an expanded on-chain prohibition list — leaving 4,753/4,800 (99.0%, warn band) with the compiler-bootstrap era ahead. **The bump is growth-based; the v0.5.9 sizing rationale is reaffirmed, not revised.** Attention quality, prompt portability, and per-token economics still govern the budget — frontier context-window growth was considered as a rationale in v0.5.9, rejected there, and remains rejected here: a 1M-token window does not make a sprawling prompt retrieve better, ship to edge models, or load cheaper at ecosystem scale. The marginal content driving the bump is high-value reference material an LLM needs to write correct Sploosh, not prose sprawl. `_WEB3` stays at `~1,500` (1,343 measured, 89.5%) — it has headroom, and raising a ceiling without pressure invites the very drift the enforcer exists to prevent. **Counterfactual considered and rejected**: recoup headroom by coarse-graining existing `_CORE` sections instead of bumping. Viable for ~50–100 tokens (precedent: the PR #40 W0010 reformulation), but the compiler bootstrap will add diagnostics and tooling references on top of an already-99% file; one honest amendment-with-rationale beats a sequence of micro-trims that erode reference quality token by token. Precedent chain: `bd26e8f` (v0.5.8, ~4,000 → ~4,800), this row (v0.5.13, → ~5,600). |
...
| v0.5.14 | **Lexer/parser spec repair — §16 EBNF aligned with promised syntax.** Splits declaration `generic_params` from use-site `type_args`, enabling nested generic types, associated type bindings (`Iter<Item = String>`), `Self::Item`, and turbofish calls. Adds `where` as a contextual keyword and formalizes `where_clause` on functions, traits, impls, structs, enums, and actors. Adds assignment expressions with restricted targets, closure-specific params for inferred and wildcard closures, `extern "C" async`, `vec![items]` / `vec![value; count]`, and `assert_matches(value, pattern)`. Numeric suffixes move into lexical productions, so `42u32` is one token and `42 u32` is not a suffixed literal. Event grammar splits top-level `onchain enum` from plain `enum Event` inside `onchain mod`. Async test prose normalized to `@test async fn`. Mirrors updated: reference grammar/keywords, prompt core note, async-test stdlib wording, and agent/spec version markers. |
| v0.5.13 | **PROMPT budget bump — §1 principle 7 amendment** (maintainer-directed). `_CORE` ceiling raised `~4,800` → `~5,600` `cl100k_base` tokens; `_WEB3` unchanged at `~1,500`. The v0.5.11/v0.5.12 amendments (contextual keywords, pipe-stage `?`, struct literals, `i128`/`u128`, expanded on-chain prohibition lists) consumed the remaining headroom — `_CORE` stood at 4,753/4,800 (99.0%, warn band) with compiler-bootstrap-era amendments ahead. Growth-based justification per the documented amendment path (precedent: v0.5.8 `bd26e8f`); **the v0.5.9 sizing rationale — attention quality, portability, per-token economics, explicitly not frontier context capacity — is reaffirmed unchanged** (new §17 row records the reaffirmation and the rejected trim-instead counterfactual). Mechanical sweep: `scripts/check_prompt_budget.py` default `--core-ceiling` `4800` → `5600` (docstring updated; `--web3-ceiling` and `--warn-at` unchanged), `docs/spec-plans/AGENTS.md` ceiling references updated. Post-bump measurements: `_CORE` 4,753/5,600 (84.9%, silent pass); `_WEB3` 1,343/1,500 (89.5%, silent pass). No language-surface changes; PROMPT file contents untouched (stamps stay at v0.5.12). |
| v0.5.12 | **Semantic reconciliation + struct-literal grammar** (closes issues #43 and #49). Eleven items: **(1)** §13.2 `std::test` target column corrected to `native (test), wasm (test)` — §13.0 already granted the assert intrinsics on wasm (test only) and `docs/stdlib/test.md` already documented wasm support; §13.2 was the lone dissenter. **(2)** §13.3.4 actor-death outcome reworded to the canonical §8.8 surface — the runner observes `Err(ActorError::Dead)` (unit variant) and reads the panic message via §8.12's `DeathCause::RuntimeFailure { panic }`; the previous `ActorError::Dead { panic: Some(msg) }` shape did not exist in §8.8 (mirror: `docs/stdlib/test.md`). **(3)** §3.1 W0010 trigger list drops `<<` / `>>` — Sploosh has no shift or bitwise operators (§2.4, §16 BINOP); registry row in `docs/reference/compiler-errors.md` updated in lockstep. **(4)** §12.3 on-chain availability lists made exhaustive against §13.2: `std::json` added to the available list; `std::log` and `std::time` added to the forbidden list (E1119 row, PROMPT prohibition lists in `_CORE` + `_WEB3`, and `docs/stdlib/{log,time}.md` updated to match). **(5)** `ActorId` gains `Debug` (§8.12.5, new §17 row) — opaque implementation-defined rendering, not to be parsed; every doc example formatting an `ActorId` moves from `{}` to `{:?}`. **(6)** §8.12.2 `by_name` example corrected to `"Worker"` (unqualified type name per the `ActorInfo.name` definition). **(7)** Pipe method-call desugar generalized (§5.6, §7.3): `expr \|> method(args)` ≡ `expr.method(args)` for any receiver whose type has the method — iterator usage is the common case, not the rule's scope (§6.5's `Option` pipes were already relying on this); stage resolution order specified (method wins over a same-named free function); addendum noted on the existing v0.5.10 §17 iter-pipe row. **(8)** Stale delegatecall / per-call-`#[gas_limit]` version stamps in §11.4a made version-neutral ("not yet specified; deferred to a future amendment") — mirrors `docs/stdlib/chain.md`, `docs/web3/cross-contract-calls.md`. **(9)** v0.5.10 Appendix D row's `_CORE` token figure annotated with the post-cubic-fix value (4,749 after `99f6e8c`). **(10)** §11.6 example corrected to `offchain async fn` — `.await` requires `async` per §8.9; `offchain` fns are not implicitly async (mirrors: `docs/web3/deploying-contracts.md`, `docs/runbooks/deploying-to-evm.md`). **(11)** **Struct literals enter §16** (closes #49): new `struct_literal = path_expr "{" field_inits "}"` expr alternative with the Rust-style **block-head restriction** as an explicit side condition — no struct literal as the outermost expression of an `if`/`while` condition, `match` scrutinee, or `for` iterable; parenthesize there. One-sentence notes in §5.1 / §5.2, new §17 row, `docs/reference/grammar.md` resynced. `Counter { state: n }` and `AppError::Timeout { after: 30 }` now derive. |
...
*This spec is a living document. v0.5.14-draft — June 2026.*
